...
The data was obtained from www.smartlab.ws. The experiments included in the data were conducted on 30 volunteers that were asked to perform 6 activities labelled (respective number lables in brackets) as WALKING (1), WALKING_UPSTAIRS(2), WALKING_DOWNSTAIRS(3), SITTING(4), STANDING(5) and LAYING(6). As they performed these activities, the subjects wore a smartphone on their waist and their movements/activities were monitored. The subjects were randomly assigned into two groups: the test group (in which 30% of the subjects were assigned) and the train group (that comprises the remaining 70% of the subjects). The data obtained were filtered and used for this analysis. The details of the experiment are given in the "README.txt" file that also describes what is included in the data set.  The data contains 561 variables measured on 30 subjects performing 6 activities. 
...
# The names of the variables are included in the "features.txt" file. We will read the file and check the dimensions of the files. 
...
# let's check the dimension of the files
dim(subject_train)
dim(X_train)
dim(y_train)
...
head(averages)
write.table(x = averages, file ="tidy_data.csv", sep = ",", row.names=FALSE)
```